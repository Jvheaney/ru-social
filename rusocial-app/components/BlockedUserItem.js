...
class MatchListItem extends Component {
  ...
  removeBlock = () => {
    ...
    return fetch('https://rumine.ca/_apiv2/gw/ft/ubu', {
      method: 'POST',
      headers: {
      ...
      .then((responseJson) => {
        if(responseJson == "logout"){
          ...
          this.props.pass.getBlockedUsers();
        }
      })
      .catch((error) => {
  ...
  render() {
    ...
    return (
          ...
          <TouchableOpacity activeOpacity={0.7} onPress={() => this.setState({areYouSure: true})} style={{position: 'absolute', right: 0, height: 50, width: 65, justifyContent: "center", alignItems: "center"}}>
            <LineIcon name="minus" size={18} color="black" />
          </TouchableOpacity>
          <View>
            <Dialog.Container visible={this.state.areYouSure}>
              <Dialog.Title>Unblock {this.props.firstname_display}</Dialog.Title>
              <Dialog.Description>
                Are you sure you want to unblock {this.props.firstname_display}?
              </Dialog.Description>
              <Dialog.Button onPress={() => this.setState({areYouSure: false})} label="Cancel" />
              <Dialog.Button onPress={() => this.removeBlock()} label="Yes" />
            </Dialog.Container>
          </View>
          </View>
   );
