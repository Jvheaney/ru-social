...
class Swipe extends Component {
  ...
  getRequests = () => {
    ...
    return fetch('https://rumine.ca/_apiv2/gw/ft/grs', {
      method: 'POST',
      headers: {
      ...
      .then((responseJson) => {
        if(responseJson == "logout"){
          ...
          alert("We could not get your sent requests right now. Please try again later.");
        }
        else{
          if(responseJson['token'] != "NA"){
            this._storeData("authToken", responseJson.token);
          ...
          cache.in_memory_cache['sent_requests'] = responseJson.status;
          this.setState({
  ...
  state = {
    requests: (cache.in_memory_cache && cache.in_memory_cache['sent_requests'])?cache.in_memory_cache['sent_requests']:[]
    }
    ...
    if(this.state.requests.length == 0){
      return(
        <View style={{height: 50, width: '100%', justifyContent: "center", alignItems: "center"}}>
          <Text style={{fontSize: 14, textAlign: "center", color: "black", fontFamily: "Raleway-Regular"}}>You have no sent requests.</Text>
        </View>
      );
    }
  return this.state.requests.map((request) => {
      return (
        <RequestItem key={request.userid} pass={this} userid={request.userid} firstname_display={request.firstname_display} image={request.image0} />
      )
    });
  }
    ...
    return (
      ...
      <StatusBar
        ...
        <View style={{backgroundColor: "white", height: 50, width: screenWidth, flexDirection: "row", alignItems: "center"}}>
          ...
          <Text style={{fontSize: 25, textAlign: "center", color: "black", fontFamily: "Raleway-Bold"}}>Sent Requests</Text>
